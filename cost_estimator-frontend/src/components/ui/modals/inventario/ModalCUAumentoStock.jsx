...
export const ModalCUAumentoStock = forwardRef((props, ref) => {
  ...
  const childFunction = (id, action, data) => {
    ...
    const filteredProducts = dataProductsProps.filter(product => product.status === 'active');
  ...
  const increaseStock = async () => {
    ...
    await axiosInstance
      .post("/api/increaseStock", dataProducts)
      .then((response) => {
        message.success("Aumento de stock exitoso");
        setOpen(false);
        props.reloadTable();
      })
      .catch((error) => {
